    ...
    <table class="dynamicTable tableTools cell-border hover table" id="listaExpedientes">
        ...
        <tbody>
            @foreach($memoEntrega->getListaEvaluaciones() as $evaluacion)
                @foreach($evaluacion->getListaEvalucionesPoligrafia() as $evaluacionPoligrafia)
                    <?php
                        $evaluacionPoligrafia->entregada() === true ? $estiloCss = 'text-success' : $estiloCss = 'text-danger';
                    ?>
                    <tr class="{{ $estiloCss }}">
                        <td>{{ $evaluacion->getElemento()->getNombreCompleto() }}</td>
                        <td>{{ $evaluacion->getElemento()->getCurp() }}</td>
                        <td>{{ $evaluacionPoligrafia->getNumeroEvaluacion() }}</td>
